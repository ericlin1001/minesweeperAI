...
char retStr[100];
PHASHDLL_API int WINAPI parseMapToFile(const char *file1, const char *file2) {

	vector<vector<int> > res;
	res = parseMineType(file1);
	ofstream out(file2);
	out << res.size() << " " << res[0].size() << endl;
...
int main(){
	...
	//showImage(img);
	vector<vector<int> > res=parseMineType("imgs\\a.jpg");
	cout << "res:";
	for (int i = 0; i < res.size(); i++) {
		for (int j = 0; j < res[i].size(); j++) {
			cout << res[i][j] << ",";
